...
export const TransactionModal = ({ isOpen, onClose }) => {
  ...
  const filteredDestinoAccounts = accounts
    .filter(
      (account) => String(account.id) !== String(cuentaOrigenSeleccionada),
    )
    .filter((account) => {
      const text = `
        ${account.tipo}
  ...
  const onSubmit = (data) => {
    showConfirmToast({
      ...
      message:
        "¿Estás seguro de realizar esta transferencia?\n\nEl dinero será transferido inmediatamente a la cuenta destino.",
      onConfirm: () =>
        submitWithFeedback({
          action: () => handleCreate(data),
          successMsg: "Transferencia realizada correctamente",
          errorMsg: "Error al realizar transferencia",
          reset,
          onClose,
        }),
    });
